    ...
    useEffect(() => {
        const handler = (ev: KeyboardEvent) => {
            ...
            if (ev.key === 'ArrowDown') {
                ...
                const item = data.find(item => item.type === 'hint' && item.activeIndex === activeIndex)!.value as ISearchResult[0]['hints'][0];
                ...
                <dl>
                    {data.map((item, i) =>
                        item.type === 'title' ? (
                            ...
                            <dd key={String(i)}>
                                <Link to={item.value.link} data-active={activeIndex === item.activeIndex || undefined} onClick={() => props.onItemSelect?.(item.value)}>
