...
class RCTree<TreeDataType extends DataNode | BasicDataNode = DataNode> extends React.Component<TreeProps<TreeDataType>, TreeState<TreeDataType>> {
    ...
    static getDerivedStateFromProps(props: TreeProps, prevState: TreeState) {
        ...
        if (treeData || newState.expandedKeys) {
            const flattenNodes = flattenTreeData<DataNode>(treeData || prevState.treeData, newState.expandedKeys || prevState.expandedKeys, fieldNames);
     ...
     */
    onNodeDragEnter = (event: React.DragEvent<HTMLDivElement>, nodeProps: TreeNodeProps<TreeDataType>) => {
    ...
    onNodeDragOver = (event: React.DragEvent<HTMLDivElement>, nodeProps: TreeNodeProps<TreeDataType>) => {
    ...
    onNodeDragLeave: NodeDragEventHandler<TreeDataType> = (event, nodeProps) => {
        ...
        if (this.currentMouseOverDroppableNodeKey === nodeProps.eventKey && !event.currentTarget.contains(event.relatedTarget as Node)) {
            this.resetDragState();
    ...
    onNodeDrop = (event: React.DragEvent<HTMLDivElement>, _: TreeNodeProps<TreeDataType>, outsideTree = false) => {
    ...
    onNodeCheck = (e: React.MouseEvent<HTMLSpanElement>, treeNode: EventDataNode<TreeDataType>, checked: boolean) => {
            ...
            this.setState(({ loadedKeys = [], loadingKeys = [] }) => {
                ...
                promise
                    ...
                    .catch(e => {
                        ...
                        this.loadingRetryTimes[key as SafeKey] = (this.loadingRetryTimes[key as SafeKey] || 0) + 1;
            ...
            if (loadPromise) {
                loadPromise
                    .then(() => {
                        ...
                        const newFlattenTreeData = flattenTreeData<TreeDataType>(this.state.treeData, expandedKeys, fieldNames);
     ...
     */
    setUncontrolledState = (state: Partial<TreeState<TreeDataType>>, atomic = false, forceState: Partial<TreeState<TreeDataType>> | null = null) => {
        if (!this.destroyed) {
            let needSync = false;
            let allPassed = true;
            const newState = {};
