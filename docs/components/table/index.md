...
## 列拖拽排序<ElTag type="primary" round={true} effect="plain" size="small">2.0.7</ElTag>

通过设置 `columnSortEnabled` 属性为 `true` ，即可开启列拖拽排序。 事件 `onColumnSortChange` 用于监听列排序变化，

<code src="./column-sort.tsx"></code>

## 行拖拽排序<ElTag type="primary" round={true} effect="plain" size="small">2.0.7</ElTag>

通过设置 `rowSortEnabled` 属性开启行拖拽排序，支持两种交互方式： `true` 表示直接拖拽表格行进行排序； `'handle'` 表示仅通过拖拽手柄（ `type='drag'` 列）进行排序。 排序结束后会触发 `onRowSortChange` 事件，回调参数包含排序后的完整数据与本次拖拽详情（起始/目标下标与被移动的行）。 建议同时设置 `rowKey` 以获得更稳定的行标识与更平滑的排序动画。 树形表格暂不支持行拖拽排序。 本示例使用 120 行数据验证大数据量下的流畅性。
...
| rowSortEnabled        | 是否开启行拖拽排序。 `true` 表示直接拖拽行排序， `'handle'` 表示仅通过拖拽手柄（type=drag 列）排序。 树形表格下无效                                                                                                     | <Enum type="boolean \| 'handle'">boolean \| 'handle'</Enum>                                                                                                        | false                                                                                       |
...
| onRowSortChange    | 拖拽行排序结束后触发， 返回排序后的完整数据及本次拖拽详情（起始/目标下标与被移动的行）                                     | <Enum type="Function">(data: T[], detail: { fromIndex: number; toIndex: number; row: T }) => void</Enum>                                                                             |
...
| 属性名              | 说明                                                                                                                                                                                                                                         | Type                                                                                                                                                                                        | 默认值                            |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| type                | 对应列的类型。 如果设置了 `selection` 则显示多选框； 如果设置了 ` index` 则显示该行的索引（从 1 开始计算）； 如果设置了 ` expand` 则显示为一个可展开的按钮； 如果设置了 ` drag` 则显示为行拖拽排序手柄（需在 Table 上设置 `rowSortEnabled`） | <Enum type="enum">'default' \| 'selection' \| 'index' \| 'expand' \| 'drag'</Enum>                                                                                                          | default                           |
