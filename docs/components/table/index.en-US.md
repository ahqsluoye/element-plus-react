...
## Column Drag Sort<ElTag type="primary" round={true} effect="plain" size="small">2.0.7</ElTag>
...
## Row Drag Sort<ElTag type="primary" round={true} effect="plain" size="small">2.0.7</ElTag>

By setting the `rowSortEnabled` attribute to `true`, you can enable row drag sort.
Supports two interaction modes: `true` enables sorting by directly dragging table rows; `'handle'` allows sorting only via the drag handle (the column with `type='drag'`). After sorting, the `onRowSortChange` event is triggered, and the callback parameters include the complete sorted data and details of the current drag operation (start/index, target index, and moved row). It is recommended to set `rowKey` for more stable row identification and smoother sorting animations. Row drag-and-drop sorting is not supported for tree tables. This example uses 120 rows to verify performance and smoothness with large datasets.

<code src="./row-sort.tsx"></code>
...
| rowSortEnabled        | Whether to enable row drag-and-drop sorting. `true` means directly dragging rows to sort, while `'handle'` means sorting only through the drag handle (type=drag column). This is not applicable for tree tables.                                                                 | <Enum type="boolean \| 'handle'">boolean \| 'handle'</Enum>                                                                                                        | false                                                                                       |
...
| onRowSortChange    | Triggered after row dragging and sorting is completed, returns the complete sorted data along with details of this drag operation (start/index, target index, and the moved rows). | <Enum type="Function">(data: T[], detail: { fromIndex: number; toIndex: number; row: T }) => void</Enum>                                                                             |
