import { ElButton, ElCol, ElForm, ElFormItem, ElFormList, ElIcon, ElInput, ElMessage, ElRow } from '@qsxy/element-plus-react';
...
const App = () => {
    ...
    return (
        <ElForm name="dynamic_form_nest_item" onFinish={onFinish} labelWidth={0} style={{ width: 600 }}>
            <ElFormList name="users">
                {(fields, { add, remove }) => (
                    <>
                        {fields.map(({ key, name, ...restField }) => (
                            <ElRow key={key} style={{ width: '100%', marginBottom: 15 }}>
                                <ElCol span={6}>
                                    <ElFormItem {...restField} name={[name, 'first']} label="姓" pure rules={[{ required: true }]}>
                                        <ElInput placeholder="请输入姓" />
                                    </ElFormItem>
                                </ElCol>
                                <ElCol span={6}>
                                    <ElFormItem {...restField} name={[name, 'last']} pure rules={[{ required: true, message: '名字不可为空' }]} labelWidth={15}>
