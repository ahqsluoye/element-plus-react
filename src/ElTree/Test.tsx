...
const TreeNode = props => {
    // 上下文注入
    const tree = useContext(ROOT_TREE_INJECTION_KEY);
    const dragEvents = useContext(dragEventsKey);

    // 状态定义
    const [expanded, setExpanded] = useState(!!props.node?.expanded);
    const [childNodeRendered, setChildNodeRendered] = useState(!!props.node?.expanded);
    const [oldChecked, setOldChecked] = useState();
    const [oldIndeterminate, setOldIndeterminate] = useState();
    ...
    }, [props.node?.data, tree?.props?.props]);
    ...
    return (
        ...
        >
            <div className={ns.be('node', 'content')} style={{ paddingLeft: contentPaddingLeft }}>
                ...
                {props.node?.loading && (
                    <span className={[ns.be('node', 'loading-icon'), ns.is('loading')]}>
                        <Loading />
                    </span>
                )}

                <NodeContent node={props.node} renderContent={props.renderContent} />
            </div>
