...
const Descriptions = memo(
    forwardRef<any, DescriptionsProps>((props, ref) => {
        ...
        /** 获取子组件 */
        const getChildInstance = useChildrenInstance<DescriptionsItemProps>('ElDescriptionsItem');

        const filledNode = useCallback(
            (node: React.ReactElement<DescriptionsItemProps>, span: number, count: number, isLast = false): React.ReactElement<DescriptionsItemProps> => {
                if (span > count) {
                ...
                <DescriptionsContext.Provider value={{ direction, border }}>
                    <div className={e`body`}>
                        <table className={classNames(e`table`, is({ bordered: border }))}>
                            <tbody>
                                {rows.map((row, index) => (
                                    <DescriptionsRow key={index} row={row} />
                                ))}
                            </tbody>
                        </table>
                    </div>
