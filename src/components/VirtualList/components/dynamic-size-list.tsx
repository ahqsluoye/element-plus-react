...
import { ItemSize, ListCache, ListItem } from '../types';
import { isHorizontal } from '../utils';

type Props = VirtualizedListProps;

const getItemFromCache = (props: Props, index: number, listCache: ListCache): ListItem => {
...
const bs = (props: Props, listCache: ListCache, low: number, high: number, offset: number) => {
    while (low <= high) {
...
const es = (props: Props, listCache: ListCache, index: number, offset: number) => {
    const { total } = props;
    let exponent = 1;
...
const getEstimatedTotalSize = ({ total }: Props, { items, estimatedItemSize, lastVisitedIndex }: ListCache) => {
...
const DynamicSizeList = createList({
    ...
    initCache({ estimatedItemSize = DEFAULT_DYNAMIC_LIST_ITEM_SIZE }) {
        const cache = {
            items: {} as Record<string | number, ListItem>,
            estimatedItemSize,
            lastVisitedIndex: -1,
        } as ListCache;
        ...
        cache.clearCacheAfterIndex = (index: number, forceUpdate = true) => {
            ...
            // In React, we'll handle force update differently
            if (forceUpdate) {
                // Force update will be handled by the component
            }
        };
    ...
    validateProps: ({ itemSize }) => {
        if (typeof itemSize !== 'function') {
            throw new Error(`[ElDynamicSizeList] itemSize is required as function, but the given value was ${typeof itemSize}`);
        }
    },
});

export type DynamicSizeListInstance = React.ElementRef<typeof DynamicSizeList> & unknown;
