...
interface useCompositionProps {
    ...
    // normalizeHandler?: (type: InputProps['nor'][number]) => InputProps['normalize'];
}
 ...
 */
export function useComposition({ value, maxLength, onChange, onKeyDown, onPressEnter }: useCompositionProps): {
    ...
    const triggerValueChange: typeof onChange = (newValue: string, e) => {
        if (
            onChange &&
