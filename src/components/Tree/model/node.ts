...
class Node {
    ...
    initialize() {
        ...
        if (this.level === 1 || this.parent?.expanded === true) {
            this.canFocus = true;
        }
    }
    ...
    updateChildren(): void {
        ...
        newData.forEach((item, index) => {
            ...
            const isNodeExists = !!key && oldData.some(data => data?.[NODE_KEY] === key);
            if (isNodeExists) {
    ...
    loadData(callback: (data?: TreeNodeData[]) => void, defaultProps: TreeNodeLoadedDefaultProps = {}) {
