...
const Collapse = memo(({ ref, ...props }: CollapseProps & { ref?: React.Ref<CollapseRef | null> }) => {
    ...
    return (
        <div className={classNames(b(), b(`icon-position-${expandIconPosition}`), props.className)} style={props.style}>
            <CollapseContext value={{ activeNames: activeName, handleItemClick }}>{props.children}</CollapseContext>
        </div>
    );
});
