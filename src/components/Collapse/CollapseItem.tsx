...
import React, { memo, use, useCallback, useMemo, useRef, useState } from 'react';
...
const CollapseItem = memo(({ ref, ...props }: CollapseItemProps & { ref?: React.Ref<CollapseItemRef | null> }) => {
    const { name, title, icon, disabled, classPrefix = 'collapse-item' } = props;
    const { b, e, is } = useClassNames(classPrefix);

    const { activeNames, handleItemClick } = use(CollapseContext);
